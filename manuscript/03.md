...
Calling `ng g lib` takes care of this path mapping. However, the implementation of the Angular CLI allows it to point to the `dist` folder and, therefore, to the compiled variant. This means the author would have to rebuild the library after every change. To avoid this annoying process, the previous listing has the mapping point to the source code version of the public. Unlike the CLI, the below-mentioned tool Nx takes care of this automatically.
...
It would be better to let the tooling figure out which applications have changed. To do this, you could calculate a hash value for all source files that flow into the application. Whenever this hash value changes, it can be assumed that the application needs to be rebuilt or tested.

Nx is a popular solution that supports this idea and comes with many additional features. In addition to Angular, it also supports other technologies such as React or Node.js-based backends, and integrates numerous tools commonly used in the development of web-based solutions. Examples include the testing tools Jest, Cypress, and Playwright, the npm server _verdaccio,_ and Storybook used for interactive component documentation. Developers do not have to invest any effort in setting up such tools but can get started straight away.

For incremental builds, Nx uses a build cache. Because Nx analyzes dependencies between the individual program parts, these mechanisms often require no manual configuration. Nx feels very natural, especially for Angular developers: The Nx CLI can be used similarly to the Angular CLI. You simply swap the `ng` instruction for `nx` - the usual arguments remain largely the same (`nx build`, `nx serve`, `nx g app`, `nx g lib`, etc.). The Nx CLI is installed via npm:
...
If the source files that flow into the affected application have not changed, you will immediately receive the result from the local cache. By default, this is located under `.nx`.
