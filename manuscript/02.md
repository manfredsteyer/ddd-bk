...
In the previous chapter, I presented strategic design which allows a software system's subdivision into loosely coupled (sub-)domains. This chapter explores these domains' implementation with Angular and an [Nx](https://nx.dev/)-based monorepo.
...
Minimizing the dependencies between individual libraries helps maintainability. This goal can be checked graphically by Nx with the ``dep-graph`` npm script:
...
> Execute the linter before checking in your source code or before merging a pull request against your main branch. If you automate this check you can enforce your architecture matrix and prevent source code that violates your architecture.
