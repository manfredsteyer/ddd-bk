...
2) The shell could put a "message bus" into the global namespace:
    ...
    Both, the shell and the Microapps could now subscribe to this message bus and listen for specific events they are interested into. Also, both can emit events.
...
![Duplicate Libraries](images/build1.png)
...
With the right wrinkles, implementing a shell for micro elements is not that difficult. However, this is only one way for implementing micro apps and -- as all -- it comes with its very own advantages and disadvantages. Hence, before implementing it, make sure it fits your architectural goals.
